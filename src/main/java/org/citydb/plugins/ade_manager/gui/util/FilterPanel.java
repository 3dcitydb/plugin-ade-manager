 ...
 */
package org.citydb.plugins.ade_manager.gui.util;
...
public class FilterPanel extends JPanel implements EventHandler {
	...
	public FilterPanel(ViewController viewController, ConfigImpl config) {
		this.config = config;
	...
	private void initGui(ViewController viewController) {
		...
		typeTree = new FeatureTypeTree(Util.toCityGMLVersion(config.getDeleteQuery().getVersion()));
